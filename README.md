...
### 12. Comment concevoir les bons parcours de notifications pour les bonnes interfaces - Arthur Retrou, head of design chez Dernier Cri

Le mot "notification" est définit dans le Larousse par l'action de faire passer un message. Pourtant, c'est un terme qui est aujourd'hui davantage évocateur d'une communication externe qui interrompt l'activité en cours. Le sons des notifications et l'incompréhension face à certaines notifications font naître frustation et énervement chez les utilisateurs. L'intérêt des notifications est pourtant très grand, autant pour le client (informatif, réception en temps réel) que pour l'annonceur (diffusion en temps réel, profits). Le but de cette conférence est donc de solutionner ce problème. Voici les cinq pistes de réflexions proposées:

1. **L'intérêt**. L'utilisateur doit toujours comprendre pourquoi il reçoit la notif et être motivé à l'ouvrir.
2. **Le message**. Une notification trop longue n'est pas une bonne notification: en moyenne, si elle dépasse 16 mots, elle n'est pas affichée. Il faut quand même qu'elle puisse fournir assez d'informations (les notifications d'Uber Eat sont un bon exemple à suivre là-dessus). Le ton du message doit aussi s'adapter aux 3C c'est-à-dire Contexte (pas de notifs Pokemon Go s'il y a une tempête par exemple), Culture (adapter à une certaine cible par exemple) et Custom (personaliser).
3.  **Le support**. Chaque support correspond à une cible. Il peut y avoir des transferts entre différents supports: les transferts mineurs (d'une pub à une page qui englobe un sujet plus large) et les transferts majeurs (d'un portable au mobile). Aborde la question des emojis dans les notifications pour comprendre très rapidement.
4.  **Le moment**. Un moment est toujours lié à un contexte: quand les gens sont en travail, on regarde en général de moins en moins nos mails et de plus en plus nos applications au fil de la journée. Certaines notifications, comme le mail de confirmation, peut déranger l'utilisateur durant son expérience sur l'applicaton en question.
5.  **La fréquence**. Il est conseillé d'éviter d'envoyer plusieurs notifications pour une même application: le mieux est d'en faire un récapitulatif condensé. On distingue deux types de fréquence de notifications: les périodiques (newsletter avec une régularité donné en amont: création d'une relation avec l'utilisateur) et les sporadiques (pas de relation donc pas de relation). Il existe quand même quelques contre-exemples amusants pour de bonnes notifications sporadiques, comme l'application Random Pizza qui envoie aléatoirement deux pizzas par mois à qui on veut, ou encore Drunk Shopping qui envoit des messages très tard pour aider à prendre de mauvaises décisions d'achat absurdes.

L'application Batch permet de faire des notifications raisonées qui mets les annonceurs face à ces questionnements. 
